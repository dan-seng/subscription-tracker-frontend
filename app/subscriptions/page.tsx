...
export default function SubscriptionsPage() {
  ...
  const fetchSubscriptions = () => {
    ...
    setIsLoading(true);
    api.get(`/api/v1/subscriptions/user/${user._id}`)
      .then((res) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-['Poppins']">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8 mt-20">
          ...
          {subs.map((sub) => (
            ...
            >
              ...
              <div className="p-6 pb-4">
                <div className="flex justify-between items-start gap-4">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <div className="flex items-center gap-1 bg-gray-50 p-1 rounded-lg">
                      <button 
                        onClick={() => router.push(`/subscriptions/${sub._id}/edit`)}
                ...
                {/* Price Section */}
                <div className="mt-5">
                  <div className="text-2xl font-bold text-gray-900">{sub.price} {sub.currency}</div>
                  <div className="text-sm text-gray-500">per {sub.frequency}</div>
                </div>
              </div>
              ...
              <div className="p-6 pt-4 space-y-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-800 font-medium">{sub.category}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Payment</span>
                  <span className="text-gray-800 font-medium">{sub.paymentMethod}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-700 font-medium">{formatDate(sub.renewalDate)}</span>
                </div>
              </div>
            </div>
          ))}
        ...
        {subs.length === 0 && (
          <p className="mt-6 text-gray-400">No subscriptions found. Add one to get started!</p>
        )}
      </div>
    </div>
  );
